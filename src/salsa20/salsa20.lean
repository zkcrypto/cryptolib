...
def x₀ : bitvec word_len := littleendian (subtype.mk [(x.nth 0), (x.nth 1), (x.nth 2), (x.nth 3)] (by refl))

-- x₁ = littleendian(x[4], x[5], x[6], x[7])
def x₁ : bitvec word_len := littleendian (subtype.mk [(x.nth 4), (x.nth 5), (x.nth 6), (x.nth 7)] (by refl))

-- x₂ = littleendian(x[8], x[9], x[10], x[11])
def x₂ : bitvec word_len := littleendian (subtype.mk [(x.nth 8), (x.nth 9), (x.nth 10), (x.nth 11)] (by refl))

-- x₃ = littleendian(x[12], x[13], x[14], x[15])
def x₃ : bitvec word_len := littleendian (subtype.mk [(x.nth 12), (x.nth 13), (x.nth 14), (x.nth 15)] (by refl))

-- x₄ = littleendian(x[16], x[17], x[18], x[19])
def x₄ : bitvec word_len := littleendian (subtype.mk [(x.nth 16), (x.nth 17), (x.nth 18), (x.nth 19)] (by refl))

-- x₅ = littleendian(x[20], x[21], x[22], x[23])
def x₅ : bitvec word_len := littleendian (subtype.mk [(x.nth 20), (x.nth 21), (x.nth 22), (x.nth 23)] (by refl))

-- x₆ = littleendian(x[24], x[25], x[26], x[27])
def x₆ : bitvec word_len := littleendian (subtype.mk [(x.nth 24), (x.nth 25), (x.nth 26), (x.nth 27)] (by refl))

-- x₇ = littleendian(x[28], x[29], x[30], x[31])
def x₇ : bitvec word_len := littleendian (subtype.mk [(x.nth 28), (x.nth 29), (x.nth 30), (x.nth 31)] (by refl))

-- x₈ = littleendian(x[32], x[33], x[34], x[35])
def x₈ : bitvec word_len := littleendian (subtype.mk [(x.nth 32), (x.nth 33), (x.nth 34), (x.nth 35)] (by refl))

-- x₉ = littleendian(x[36], x[37], x[38], x[39])
def x₉ : bitvec word_len := littleendian (subtype.mk [(x.nth 36), (x.nth 37), (x.nth 38), (x.nth 39)] (by refl))

-- x₁₀ = littleendian(x[40], x[41], x[42], x[43])
def x₁₀ : bitvec word_len := littleendian (subtype.mk [(x.nth 40), (x.nth 41), (x.nth 42), (x.nth 43)] (by refl))

-- x₁₁ = littleendian(x[44], x[45], x[46], x[47])
def x₁₁ : bitvec word_len := littleendian (subtype.mk [(x.nth 44), (x.nth 45), (x.nth 46), (x.nth 47)] (by refl))

-- x₁₂ = littleendian(x[48], x[49], x[50], x[51])
def x₁₂ : bitvec word_len := littleendian (subtype.mk [(x.nth 48), (x.nth 49), (x.nth 50), (x.nth 51)] (by refl))

-- x₁₃ = littleendian(x[52], x[53], x[54], x[55])
def x₁₃ : bitvec word_len := littleendian (subtype.mk [(x.nth 52), (x.nth 53), (x.nth 54), (x.nth 55)] (by refl))

-- x₁₄ = littleendian(x[56], x[57], x[58], x[59])
def x₁₄ : bitvec word_len := littleendian (subtype.mk [(x.nth 56), (x.nth 57), (x.nth 58), (x.nth 59)] (by refl))

-- x₁₅ = littleendian(x[60], x[61], x[62], x[63])
def x₁₅ : bitvec word_len := littleendian (subtype.mk [(x.nth 60), (x.nth 61), (x.nth 62), (x.nth 63)] (by refl))
