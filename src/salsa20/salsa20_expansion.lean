...
-- σ₀ = (101, 120, 112, 97)
def σ₀ : vector (bitvec byte_len) 4 := 
  ([101, 120, 112, 97].to_vec_of_bitvec byte_len 4)

-- σ₁ = (110, 100, 32, 51)
def σ₁ : vector (bitvec byte_len) 4 := 
  ([110, 100, 32, 51].to_vec_of_bitvec byte_len 4)

-- σ₂ = (50, 45, 98, 121)
def σ₂ : vector (bitvec byte_len) 4 := 
  ([50, 45, 98, 121].to_vec_of_bitvec byte_len 4)

-- σ₃ = (116, 101, 32, 107)
def σ₃ : vector (bitvec byte_len) 4 := 
  ([116, 101, 32, 107].to_vec_of_bitvec byte_len 4)

-- τ₀ = (101, 120, 112, 97)
def τ₀ : vector (bitvec byte_len) 4 := 
  ([101, 120, 112, 97].to_vec_of_bitvec byte_len 4)

-- τ₁ = (110, 100, 32, 49)
def τ₁ : vector (bitvec byte_len) 4 := 
  ([110, 100, 32, 49].to_vec_of_bitvec byte_len 4)

-- τ₂ = (54, 45, 98, 121)
def τ₂ : vector (bitvec byte_len) 4 := 
  ([54, 45, 98, 121].to_vec_of_bitvec byte_len 4)
...
def salsa20_expansion_v1 (k₀ : vector (bitvec byte_len) 16)
  (k₁ : vector (bitvec byte_len) 16) (n : vector (bitvec byte_len) 16) : vector (bitvec byte_len) 64 :=
    salsa20.salsa20 
      ((((((σ₀.append k₀).append σ₁).append n)
                .append σ₂).append k₁).append σ₃)
...
def salsa20_expansion_v2 (k : vector (bitvec byte_len) 16)
  (n : vector (bitvec byte_len) 16) : vector (bitvec byte_len) 64 :=
    salsa20.salsa20 
      ((((((τ₀.append k).append τ₁).append n)
                .append τ₂).append k).append τ₃)
...
def res2 : list(bitvec byte_len) := [
  39, 173, 46, 248, 30, 200, 82, 17,
  48, 67, 254, 239, 37, 18, 13, 247,
  241, 200, 61, 144, 10, 55, 50, 185, 6,
  47, 246, 253, 143, 86, 187, 225,
  134, 85, 110, 246, 161, 163, 43, 235, 231,
  94, 171, 51, 145, 214, 112, 29,
  14, 232, 5, 16, 151, 140, 183, 141,
  171, 9, 122, 181, 104, 182, 177, 193
]

#eval if res1 = (
    salsa20_expansion_v1 test_k₀ test_k₁ test_n
  ).to_list then tt else ff

#eval if res2 = (
    salsa20_expansion_v2 test_k₀ test_n
  ).to_list then tt else ff
