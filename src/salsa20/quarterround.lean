...
def z₁ (y : vector (bitvec word_len) 4) : bitvec word_len := 
  bitvec.xor (y.nth 1) (rotl (nat_as_bitvec (mod_as_nat (sum_as_mod (y.nth 0) (y.nth 3)))) 7)

-- z₂ = y₂ ⊕ ((z₁ + y₀) <<< 9)
def z₂ (y : vector (bitvec word_len) 4) : bitvec word_len := 
  bitvec.xor (y.nth 2) (rotl (nat_as_bitvec (mod_as_nat (sum_as_mod (z₁ y) (y.nth 0)))) 9)

-- z₃ = y₃ ⊕ ((z₂ + z₁) <<< 13)
def z₃ (y : vector (bitvec word_len) 4) : bitvec word_len := 
  bitvec.xor (y.nth 3) (rotl (nat_as_bitvec (mod_as_nat (sum_as_mod (z₂ y) (z₁ y)))) 13)

-- z₀ = y₀ ⊕ ((z₃ + z₂) <<< 18)
def z₀ (y : vector (bitvec word_len) 4) : bitvec word_len := 
  bitvec.xor (y.nth 0) (rotl (nat_as_bitvec (mod_as_nat (sum_as_mod (z₃ y) (z₂ y)))) 18)
