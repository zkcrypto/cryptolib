...
instance fintype : Π (n : ℕ), Fintype (BitVec n) := by
  intro n
  sorry
...
-- instance group (n : ℕ) : Group (ZMod n) := by
instance group : ∀ (n : ℕ) [NeZero n], Group (ZMod n) := by
  intros n h
  sorry
...
--     ⟨List.take len_vec l, List.take_length len_vec l⟩
